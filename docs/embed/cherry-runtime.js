document.addEventListener('DOMContentLoaded', () => {
    ...
    const isWidgetEmbedded = isEmbeddedApp && !isInlineEmbedded;
    ...
    const gitDeployBtn = document.getElementById('git-deploy-menu-btn');
    const deployCredsBtn = document.getElementById('deploy-github-btn');
    const deployHostingerBtn = document.getElementById('deploy-hostinger-btn');
    ...
    const gitDeployDrawer = document.getElementById('git-deploy-drawer');
    const gitDeployOverlay = document.getElementById('git-deploy-overlay');
    const gitDeployClose = document.getElementById('git-deploy-close');
    const hostingerSiteUrlInput = document.getElementById('hostinger-site-url');
    const hostingerHostInput = document.getElementById('hostinger-host');
    const hostingerPortInput = document.getElementById('hostinger-port');
    const hostingerUsernameInput = document.getElementById('hostinger-username');
    const hostingerPasswordInput = document.getElementById('hostinger-password');
    const hostingerRemoteDirInput = document.getElementById('hostinger-remote-dir');
    const hostingerSecureSelect = document.getElementById('hostinger-secure');
    const saveHostingerSettingsBtn = document.getElementById('save-hostinger-settings');
    const saveHostingerPasswordBtn = document.getElementById('save-hostinger-password');
    const clearHostingerPasswordBtn = document.getElementById('clear-hostinger-password');
    const testHostingerAccessBtn = document.getElementById('test-hostinger-access');
    ...
    const gitDeployStatus = document.getElementById('git-deploy-status');
    ...
    const authModeButtons = document.querySelectorAll('.auth-mode-btn[data-auth-mode]');
    ...
    const authHeaderFields = document.getElementById('auth-header-fields');
    const authBasicUsernameInput = document.getElementById('auth-basic-username');
    const authBasicPasswordInput = document.getElementById('auth-basic-password');
    const authJwtTokenInput = document.getElementById('auth-jwt-token');
    const authHeaderNameInput = document.getElementById('auth-header-name');
    const authHeaderValueInput = document.getElementById('auth-header-value');
    ...
    const converseInputBarVisibleInput = document.getElementById('converse-input-bar-visible');
    ...
    const videoImageUrlInput = document.getElementById('video-image-url');
    const videoImageFileInput = document.getElementById('video-image-file');
    const videoImageEnabledInput = document.getElementById('video-image-enabled');
    const videoSizeInput = document.getElementById('video-size');
    ...
    const headerVideoImage = document.getElementById('header-video-image');
    ...
    const questionShowInputBarInput = document.getElementById('question-show-input-bar');
    ...
    const telemetryActiveMode = document.getElementById('telemetry-active-mode');
    const telemetryTargetBadge = document.getElementById('telemetry-target-badge');
    const telemetryTargetSummary = document.getElementById('telemetry-target-summary');
    ...
    const telemetryTargetButtons = document.querySelectorAll('.telemetry-target-btn[data-target]');
    ...
    const GITHUB_STATIC_DIR = 'docs/embed';
    const GITHUB_LOADER_FILENAME = 'cherry-embed-loader.js';
    const GITHUB_RUNTIME_HTML_FILENAME = 'cherry-runtime.html';
    const GITHUB_RUNTIME_JS_FILENAME = 'cherry-runtime.js';
    const GITHUB_RUNTIME_CSS_FILENAME = 'cherry-runtime.css';
    const GITHUB_ASSET_FILENAMES = [
        'astig_logo.png',
        'character_standin.jpg',
        'character_standin.png',
        'cherry-logo.png',
        'couple_photo.png',
        'favicon.png',
        'floral_motif.png',
    ...
    const DEFAULT_EMBED_HOSTED_BASE = (() => {
        try {
    ...
    const DEFAULT_EMBED_RUNTIME_BASE = DEFAULT_EMBED_HOSTED_BASE;
    const DEFAULT_EMBED_APP_BASE = DEFAULT_EMBED_HOSTED_BASE;
    ...
    const IS_LOCAL_BUILDER = ['localhost', '127.0.0.1'].includes(window.location.hostname);
    const LOCAL_BUILDER_PORTS = ['8000', '8010', '8011', '8012', '8080'];

    const STORAGE_KEYS = {
        repo: 'cherry.github.repo',
        branch: 'cherry.github.branch',
        token: 'cherry.github.token',
        hostingerSiteUrl: 'cherry.hostinger.siteUrl',
        hostingerHost: 'cherry.hostinger.host',
        hostingerPort: 'cherry.hostinger.port',
        hostingerUsername: 'cherry.hostinger.username',
        hostingerPassword: 'cherry.hostinger.password',
        hostingerRemoteDir: 'cherry.hostinger.remoteDir',
        hostingerSecure: 'cherry.hostinger.secure',
        ...
        authMode: 'cherry.auth.mode',
        authBasicUsername: 'cherry.auth.basic.username',
        authBasicPassword: 'cherry.auth.basic.password',
        authJwtToken: 'cherry.auth.jwt.token',
        authHeaderName: 'cherry.auth.header.name',
        authHeaderValue: 'cherry.auth.header.value',
        ...
        inputBarVisible: 'cherry.inputBar.visible',
        ...
        videoImageUrl: 'cherry.video.imageUrl',
        videoImageEnabled: 'cherry.video.imageEnabled',
        videoSize: 'cherry.video.size',
        videoPositionX: 'cherry.video.positionX',
        videoPositionY: 'cherry.video.positionY',
    ...
    function sanitizeAuthSettings(settings = {}) {
        const defaults = getDefaultAuthSettings();
        const mode = ['none', 'basic', 'jwt', 'header'].includes(settings.mode) ? settings.mode : defaults.mode;
        return {
            mode,
            basicUsername: String(settings.basicUsername ?? defaults.basicUsername),
            basicPassword: String(settings.basicPassword ?? defaults.basicPassword),
            jwtToken: String(settings.jwtToken ?? defaults.jwtToken),
            headerName: String(settings.headerName ?? defaults.headerName),
            headerValue: String(settings.headerValue ?? defaults.headerValue)
        };
    }
    ...
    function describeAuthMode(mode) {
        switch (mode) {
            case 'basic':
                return 'Basic Auth is active for n8n webhook requests.';
            case 'jwt':
                return 'JWT Bearer auth is active for n8n webhook requests.';
            case 'header':
                return 'Header Auth is active for n8n webhook requests.';
            default:
                return 'No webhook authorization is active.';
        }
    }
    ...
    function updateAuthModeUi(mode) {
        ...
        if (authBasicFields) authBasicFields.hidden = mode !== 'basic';
        if (authJwtFields) authJwtFields.hidden = mode !== 'jwt';
        if (authHeaderFields) authHeaderFields.hidden = mode !== 'header';
    }

    function updateAuthStatus(message, tone = 'success') {
    ...
    function getActiveAuthMode() {
        return Array.from(authModeButtons).find(btn => btn.classList.contains('active'))?.dataset.authMode
            || safeStorageGet(STORAGE_KEYS.authMode, getDefaultAuthSettings().mode)
            || getDefaultAuthSettings().mode;
    }

    function getStoredAuthSettings() {
        return sanitizeAuthSettings({
            mode: getActiveAuthMode(),
            basicUsername: authBasicUsernameInput?.value ?? safeStorageGet(STORAGE_KEYS.authBasicUsername, ''),
            basicPassword: authBasicPasswordInput?.value ?? safeStorageGet(STORAGE_KEYS.authBasicPassword, ''),
            jwtToken: authJwtTokenInput?.value ?? safeStorageGet(STORAGE_KEYS.authJwtToken, ''),
            headerName: authHeaderNameInput?.value ?? safeStorageGet(STORAGE_KEYS.authHeaderName, 'Authorization'),
            headerValue: authHeaderValueInput?.value ?? safeStorageGet(STORAGE_KEYS.authHeaderValue, '')
        });
    }

    function hydrateAuthSettings() {
        const settings = sanitizeAuthSettings({
            mode: safeStorageGet(STORAGE_KEYS.authMode, getDefaultAuthSettings().mode),
            basicUsername: safeStorageGet(STORAGE_KEYS.authBasicUsername, ''),
            basicPassword: safeStorageGet(STORAGE_KEYS.authBasicPassword, ''),
            jwtToken: safeStorageGet(STORAGE_KEYS.authJwtToken, ''),
            headerName: safeStorageGet(STORAGE_KEYS.authHeaderName, 'Authorization'),
            headerValue: safeStorageGet(STORAGE_KEYS.authHeaderValue, '')
        });

        if (authBasicUsernameInput) authBasicUsernameInput.value = settings.basicUsername;
        if (authBasicPasswordInput) authBasicPasswordInput.value = settings.basicPassword;
        if (authJwtTokenInput) authJwtTokenInput.value = settings.jwtToken;
        if (authHeaderNameInput) authHeaderNameInput.value = settings.headerName;
        if (authHeaderValueInput) authHeaderValueInput.value = settings.headerValue;
        updateAuthModeUi(settings.mode);
        updateAuthStatus(describeAuthMode(settings.mode));
    }

    function saveAuthSettings(showFeedback = true) {
        try {
            const settings = getStoredAuthSettings();
            safeStorageSet(STORAGE_KEYS.authMode, settings.mode);
            safeStorageSet(STORAGE_KEYS.authBasicUsername, settings.basicUsername);
            safeStorageSet(STORAGE_KEYS.authBasicPassword, settings.basicPassword);
            safeStorageSet(STORAGE_KEYS.authJwtToken, settings.jwtToken);
            safeStorageSet(STORAGE_KEYS.authHeaderName, settings.headerName);
            safeStorageSet(STORAGE_KEYS.authHeaderValue, settings.headerValue);
            updateAuthModeUi(settings.mode);
            if (showFeedback && saveAuthSettingsBtn) flashButton(saveAuthSettingsBtn, 'Saved');
            updateAuthStatus(showFeedback ? `Saved. ${describeAuthMode(settings.mode)}` : describeAuthMode(settings.mode), 'success');
        } catch (_) {
            updateAuthStatus('Could not save auth settings (storage blocked).', 'error');
        }
    }
    ...
    function getWebhookAuthHeaders() {
        ...
        if (settings.mode === 'basic') {
            if (!settings.basicUsername && !settings.basicPassword) return {};
            return {
                Authorization: `Basic ${encodeBasicAuth(`${settings.basicUsername}:${settings.basicPassword}`)}`
            };
        }
        if (settings.mode === 'jwt') {
            const jwtToken = settings.jwtToken.trim();
            if (!jwtToken) return {};
            return {
        ...
        if (settings.mode === 'header') {
            const headerName = settings.headerName.trim();
            const headerValue = settings.headerValue.trim();
    ...
    function getWebhookModeMeta(mode) {
        switch (mode) {
            case 'test':
                return {
                    label: 'TEST URL',
                    summary: 'Testing endpoint is active for realtime monitoring.'
                };
            case 'chat':
                return {
                    label: 'CHAT URL',
                    summary: 'Chat endpoint is active for realtime monitoring.'
                };
            default:
                return {
                    label: 'PROD URL',
                    summary: 'Production endpoint is active for realtime monitoring.'
                };
        }
    }

    function getActiveWebhookMode() {
        return safeStorageGet(STORAGE_KEYS.webhookActive, 'prod') || 'prod';
    }

    function renderTelemetry() {
        const activeWebhookMode = getActiveWebhookMode();
        const activeWebhookMeta = getWebhookModeMeta(activeWebhookMode);
        ...
        if (telemetryActiveMode) telemetryActiveMode.textContent = activeWebhookMeta.label;
        if (telemetryTargetBadge) telemetryTargetBadge.textContent = 'ACTIVE';
        if (telemetryTargetSummary) telemetryTargetSummary.textContent = activeWebhookMeta.summary;
        ...
        telemetryTargetButtons.forEach(btn => {
            btn.classList.toggle('active', btn.dataset.target === activeWebhookMode);
        });
    ...
    if (deployBtn) {
        deployBtn.addEventListener('click', handlePrimaryDeployAction);
    }
    ...
    if (deployHostingerBtn) {
        deployHostingerBtn.addEventListener('click', deployToHostinger);
    }
    ...
    if (gitDeployBtn) {
        gitDeployBtn.addEventListener('click', () => openGitDeployDrawer(true));
    }

    if (gitDeployOverlay) {
        gitDeployOverlay.addEventListener('click', () => openGitDeployDrawer(false));
    }

    if (gitDeployClose) {
        gitDeployClose.addEventListener('click', () => openGitDeployDrawer(false));
    }
    ...
    if (authBtn) {
        authBtn.addEventListener('click', () => openAuthDrawer(true));
    }

    if (authOverlay) {
        authOverlay.addEventListener('click', () => openAuthDrawer(false));
    }

    if (authClose) {
        authClose.addEventListener('click', () => openAuthDrawer(false));
    }

    authModeButtons.forEach(btn => {
        btn.addEventListener('click', () => updateAuthModeUi(btn.dataset.authMode));
    });
    ...
    if (videoImageUrlInput) {
        videoImageUrlInput.addEventListener('input', previewVideoSettings);
    }
    if (videoImageFileInput) {
        videoImageFileInput.addEventListener('change', handleVideoImageFileSelect);
    }
    if (videoImageEnabledInput) {
        videoImageEnabledInput.addEventListener('change', previewVideoSettings);
    }
    if (videoSizeInput) {
        videoSizeInput.addEventListener('input', previewVideoSettings);
    }
    if (videoEnabledInput) {
        videoEnabledInput.addEventListener('change', previewVideoSettings);
    }
    if (headerVideoImage) {
        headerVideoImage.addEventListener('load', () => {
            headerVideoImage.hidden = false;
            headerVideoImage.classList.remove('is-broken');
        });
        headerVideoImage.addEventListener('error', () => {
            headerVideoImage.hidden = true;
            headerVideoImage.classList.add('is-broken');
            if (headerVideo && headerVideoPlayer?.hidden !== false) {
                setHeaderMediaVisible(false);
            }
        });
    }
    if (headerVideo) {
        headerVideo.addEventListener('pointerdown', beginHeaderMediaDrag);
        headerVideo.addEventListener('pointermove', handleHeaderMediaDrag);
        headerVideo.addEventListener('pointerup', endHeaderMediaDrag);
        headerVideo.addEventListener('pointercancel', endHeaderMediaDrag);
        headerVideo.addEventListener('lostpointercapture', endHeaderMediaDrag);
    }
    if (logoTargetSelect) {
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            ...
            if (gitDeployDrawer?.classList.contains('open')) openGitDeployDrawer(false);
            if (webhookDrawer?.classList.contains('open')) openWebhookDrawer(false);
            if (authDrawer?.classList.contains('open')) openAuthDrawer(false);
    ...
    function hydrateQuestionnaireSettings() {
        ...
        questionnaireSettings = {
            ...
            inputBarVisible: safeStorageGet(STORAGE_KEYS.inputBarVisible, String(getDefaultQuestionnaireSettings().inputBarVisible)) !== 'false'
        };
    ...
    function sanitizeVideoSettings(candidate = {}) {
        ...
        const numericSize = Number(candidate?.size);
        const numericPositionX = Number(candidate?.positionX);
        const numericPositionY = Number(candidate?.positionY);
        return {
            enabled: typeof candidate?.enabled === 'boolean' ? candidate.enabled : true,
            url: String(candidate?.url || defaults.url).trim(),
            imageUrl: String(candidate?.imageUrl || defaults.imageUrl).trim(),
            imageEnabled: typeof candidate?.imageEnabled === 'boolean' ? candidate.imageEnabled : defaults.imageEnabled,
            size: Number.isFinite(numericSize) ? Math.min(220, Math.max(60, numericSize)) : defaults.size,
            positionX: Number.isFinite(numericPositionX) ? Math.min(100, Math.max(0, numericPositionX)) : defaults.positionX,
            positionY: Number.isFinite(numericPositionY) ? Math.min(100, Math.max(0, numericPositionY)) : defaults.positionY
        };
    }
    ...
    function handleVideoImageFileSelect(event) {
        const file = event?.target?.files?.[0];
        if (!file) return;
        if (!file.type.startsWith('image/')) {
            setVideoStatus('Choose an image file for the header media.', 'error');
            return;
        }
        const reader = new FileReader();
        reader.onload = () => {
            if (videoImageUrlInput) {
                videoImageUrlInput.value = typeof reader.result === 'string' ? reader.result : '';
            }
            if (videoImageEnabledInput) {
                videoImageEnabledInput.checked = true;
            }
            previewVideoSettings();
            setVideoStatus('Local image loaded into the header media preview.', 'success');
        };
        reader.onerror = () => {
            setVideoStatus('Could not read that image file.', 'error');
        };
        reader.readAsDataURL(file);
    }

    function setHeaderMediaVisible(visible) {
        if (!headerVideo) return;
        headerVideo.hidden = !visible;
        headerVideo.style.display = visible ? 'block' : 'none';
        headerVideo.setAttribute('aria-hidden', visible ? 'false' : 'true');
    }
    ...
    function applyVideoSettings(settings) {
        ...
        const maxOffsetY = (nextSettings.size || headerVideo.clientHeight || getDefaultVideoSettings().size) * 0.35;
        const translateX = ((nextSettings.positionX - 50) / 50) * maxOffsetX;
        const translateY = ((nextSettings.positionY - 50) / 50) * maxOffsetY;
        headerVideo.style.setProperty('--header-media-translate-x', `${translateX}px`);
        headerVideo.style.setProperty('--header-media-translate-y', `${translateY}px`);
        const resolvedVideoUrl = resolveMediaUrl(nextSettings.url);
        const resolvedImageUrl = resolveMediaUrl(nextSettings.imageUrl);
        const hasVideoUrl = nextSettings.enabled && !!resolvedVideoUrl && isValidMediaUrl(nextSettings.url);
        const hasImageUrl = nextSettings.imageEnabled && !hasVideoUrl && !!resolvedImageUrl && isValidMediaUrl(nextSettings.imageUrl);
        ...
        setHeaderMediaVisible(true);
        headerVideo.classList.toggle('is-draggable-media', !!saveVideoSettingsBtn && (hasVideoUrl || hasImageUrl));
        ...
        headerVideoPlayer.hidden = !hasVideoUrl;
        headerVideoImage.hidden = !hasImageUrl;
        if (hasVideoUrl) {
            headerVideoPlayer.src = resolvedVideoUrl;
            headerVideoImage.classList.remove('is-broken');
            headerVideoImage.removeAttribute('src');
        ...
        if (hasImageUrl) {
            headerVideoImage.classList.remove('is-broken');
            headerVideoImage.src = resolvedImageUrl;
        } else {
            headerVideoImage.classList.remove('is-broken');
            headerVideoImage.removeAttribute('src');
        }
    ...
    function hydrateVideoSettings() {
        const settings = sanitizeVideoSettings({
            ...
            imageUrl: safeStorageGet(STORAGE_KEYS.videoImageUrl, getDefaultVideoSettings().imageUrl),
            imageEnabled: safeStorageGet(STORAGE_KEYS.videoImageEnabled, 'true') !== 'false',
            size: safeStorageGet(STORAGE_KEYS.videoSize, String(getDefaultVideoSettings().size)),
            positionX: safeStorageGet(STORAGE_KEYS.videoPositionX, String(getDefaultVideoSettings().positionX)),
            positionY: safeStorageGet(STORAGE_KEYS.videoPositionY, String(getDefaultVideoSettings().positionY))
        });
        if (videoEnabledInput) videoEnabledInput.checked = settings.enabled;
        if (videoUrlInput) videoUrlInput.value = settings.url;
        if (videoImageUrlInput) videoImageUrlInput.value = settings.imageUrl;
        if (videoImageEnabledInput) videoImageEnabledInput.checked = settings.imageEnabled;
        if (videoSizeInput) videoSizeInput.value = String(settings.size);
    ...
    function previewVideoSettings() {
        const settings = sanitizeVideoSettings({
            ...
            imageUrl: videoImageUrlInput?.value || '',
            imageEnabled: !!videoImageEnabledInput?.checked,
            size: videoSizeInput?.value || getDefaultVideoSettings().size,
            positionX: safeStorageGet(STORAGE_KEYS.videoPositionX, String(getDefaultVideoSettings().positionX)),
            positionY: safeStorageGet(STORAGE_KEYS.videoPositionY, String(getDefaultVideoSettings().positionY))
        });
        if (settings.url && !isValidMediaUrl(settings.url)) {
            setVideoStatus('Enter a valid video or asset URL.', 'error');
            return false;
        }
        if (settings.imageUrl && !isValidMediaUrl(settings.imageUrl)) {
            setVideoStatus('Enter a valid image or asset URL.', 'error');
            return false;
        }
        applyVideoSettings(settings);
        setVideoStatus((settings.enabled && settings.url) || (settings.imageEnabled && settings.imageUrl) ? 'Header media preview updated under the chatbot header.' : 'Header media hidden from the chatbot header.', 'success');
        return true;
    }
    ...
    function saveVideoSettings() {
        ...
        safeStorageSet(STORAGE_KEYS.videoImageUrl, settings.imageUrl);
        safeStorageSet(STORAGE_KEYS.videoImageEnabled, String(settings.imageEnabled));
        safeStorageSet(STORAGE_KEYS.videoSize, String(settings.size));
        safeStorageSet(STORAGE_KEYS.videoPositionX, String(settings.positionX));
        safeStorageSet(STORAGE_KEYS.videoPositionY, String(settings.positionY));
        applyVideoSettings(settings);
        flashButton(saveVideoSettingsBtn, 'Applied');
        setVideoStatus((settings.enabled && settings.url) || (settings.imageEnabled && settings.imageUrl) ? 'Header media saved to the builder preview.' : 'Header media hidden.', 'success');
    }

    function updateHeaderMediaPositionFromPointer(clientX, clientY) {
        if (!headerVideo) return;
        const rect = headerVideo.getBoundingClientRect();
        if (!rect.width || !rect.height) return;
        const positionX = Math.min(100, Math.max(0, ((clientX - rect.left) / rect.width) * 100));
        const positionY = Math.min(100, Math.max(0, ((clientY - rect.top) / rect.height) * 100));
        safeStorageSet(STORAGE_KEYS.videoPositionX, String(positionX));
        safeStorageSet(STORAGE_KEYS.videoPositionY, String(positionY));
        applyVideoSettings({
            enabled: !!videoEnabledInput?.checked,
            url: videoUrlInput?.value || '',
            imageUrl: videoImageUrlInput?.value || '',
    ...
    function beginHeaderMediaDrag(event) {
        if (!saveVideoSettingsBtn || !headerVideo || headerVideo.hidden) return;
        if (event.button !== 0 && event.pointerType !== 'touch') return;
        activeMediaDragPointerId = event.pointerId;
        headerVideo.classList.add('is-dragging-media');
        headerVideo.setPointerCapture?.(event.pointerId);
        updateHeaderMediaPositionFromPointer(event.clientX, event.clientY);
        event.preventDefault();
    }

    function handleHeaderMediaDrag(event) {
        if (activeMediaDragPointerId !== event.pointerId) return;
        updateHeaderMediaPositionFromPointer(event.clientX, event.clientY);
    }

    function endHeaderMediaDrag(event) {
        if (activeMediaDragPointerId !== null && event.pointerId !== activeMediaDragPointerId) return;
        activeMediaDragPointerId = null;
        headerVideo?.classList.remove('is-dragging-media');
    }
    ...
    function isValidMediaUrl(value) {
        ...
        if (!url) return false;
        try {
            const parsed = new URL(normalizeMediaUrl(value), document.baseURI || window.location.href);
            return ['http:', 'https:', 'data:', 'blob:'].includes(parsed.protocol) || !url.includes('://');
        } catch (_) {
            return false;
        }
    }
    ...
    function normalizeMediaUrl(value) {
        ...
        if (!rawUrl) return '';
        const slashNormalized = rawUrl.replace(/\\/g, '/');
        const assetsMatch = slashNormalized.match(/(?:^|\/)(assets\/.+)$/i);
        if (assetsMatch) {
            return assetsMatch[1];
        }
        if (/^[a-zA-Z]:[\\/]/.test(rawUrl)) {
            return `file:///${slashNormalized}`;
        }
    ...
    function saveFooterSettings() {
        ...
        setFooterStatus(
            settings.url && !isValidHttpUrl(settings.url)
                ? 'Footer branding applied. The text will show without a clickable link until the URL is valid.'
                : 'Footer branding applied to the builder preview.',
            'success'
        );
    }
    ...
    function sanitizeConverseSettings(candidate = {}) {
        ...
        return {
            inputBarVisible: typeof candidate?.inputBarVisible === 'boolean' ? candidate.inputBarVisible : defaults.inputBarVisible,
            intro: {
    ...
    function applyConverseSettings(settings) {
        ...
        currentConverseSettings = nextSettings;
        applyInputBarVisibility(nextSettings.inputBarVisible);
        if (introScreen) {
    ...
    function hydrateConverseSettings() {
        const settings = sanitizeConverseSettings({
            inputBarVisible: safeStorageGet(STORAGE_KEYS.inputBarVisible, String(getDefaultConverseSettings().inputBarVisible)) !== 'false',
            intro: {
        ...
        if (converseInputBarVisibleInput) converseInputBarVisibleInput.checked = settings.inputBarVisible;
    ...
    function saveConverseSettings() {
        const settings = sanitizeConverseSettings({
            inputBarVisible: !!converseInputBarVisibleInput?.checked,
            intro: {
        ...
        safeStorageSet(STORAGE_KEYS.inputBarVisible, String(settings.inputBarVisible));
    ...
    function getProjectSnapshot() {
        ...
        return {
            ...
            video: sanitizeVideoSettings({
                ...
                imageUrl: videoImageUrlInput?.value || safeStorageGet(STORAGE_KEYS.videoImageUrl, getDefaultVideoSettings().imageUrl),
                imageEnabled: videoImageEnabledInput?.checked ?? (safeStorageGet(STORAGE_KEYS.videoImageEnabled, 'true') !== 'false'),
                size: videoSizeInput?.value || safeStorageGet(STORAGE_KEYS.videoSize, String(getDefaultVideoSettings().size)),
                positionX: safeStorageGet(STORAGE_KEYS.videoPositionX, String(getDefaultVideoSettings().positionX)),
                positionY: safeStorageGet(STORAGE_KEYS.videoPositionY, String(getDefaultVideoSettings().positionY))
            }),
    ...
    function getFreshProjectSnapshot() {
        return {
            ...
            webhook: {
                prod: WEBHOOK_URL_PROD,
                test: WEBHOOK_URL_TEST,
                chat: '',
                active: 'prod'
            },
            auth: getDefaultAuthSettings(),
            embed: {
                runtimeBase: DEFAULT_EMBED_RUNTIME_BASE,
                appBase: DEFAULT_EMBED_APP_BASE,
                commit: CURRENT_EMBED_COMMIT
            },
            github: {
    ...
    function createBlankProjectSnapshot() {
        return {
            questionnaire: {
                ...
                settings: {
                    ...getDefaultQuestionnaireSettings(),
            ...
            converse: sanitizeConverseSettings({
                ...
                intro: {
                    ...
                    animation: 'none',
                    title: '',
                    titleFontFamily: getDefaultConverseSettings().intro.titleFontFamily,
                    titleFontSize: getDefaultConverseSettings().intro.titleFontSize,
                    message: '',
                    messageFontFamily: getDefaultConverseSettings().intro.messageFontFamily,
                    messageFontSize: getDefaultConverseSettings().intro.messageFontSize,
                    buttonText: '',
                    buttonFontFamily: getDefaultConverseSettings().intro.buttonFontFamily,
                    buttonFontSize: getDefaultConverseSettings().intro.buttonFontSize
                },
                assistant: getDefaultConverseSettings().assistant,
                user: getDefaultConverseSettings().user
            }),
            theme: sanitizeThemeSettings({
    ...
    function persistBlankProjectStorage(snapshot) {
        const authSettings = sanitizeAuthSettings(snapshot?.auth || getDefaultAuthSettings());
        ...
        safeStorageSet(STORAGE_KEYS.inputBarVisible, String(snapshot.questionnaire.settings.inputBarVisible !== false));
        ...
        safeStorageSet(STORAGE_KEYS.videoImageUrl, snapshot.video.imageUrl);
        safeStorageSet(STORAGE_KEYS.videoImageEnabled, String(snapshot.video.imageEnabled));
        safeStorageSet(STORAGE_KEYS.videoSize, String(snapshot.video.size));
        safeStorageSet(STORAGE_KEYS.videoPositionX, String(snapshot.video.positionX));
        safeStorageSet(STORAGE_KEYS.videoPositionY, String(snapshot.video.positionY));
        ...
        safeStorageSet(STORAGE_KEYS.authMode, authSettings.mode);
        safeStorageSet(STORAGE_KEYS.authBasicUsername, authSettings.basicUsername);
        safeStorageSet(STORAGE_KEYS.authBasicPassword, authSettings.basicPassword);
        safeStorageSet(STORAGE_KEYS.authJwtToken, authSettings.jwtToken);
        safeStorageSet(STORAGE_KEYS.authHeaderName, authSettings.headerName);
        safeStorageSet(STORAGE_KEYS.authHeaderValue, authSettings.headerValue);
    ...
    function createNewProject() {
        try {
            const blankSnapshot = createBlankProjectSnapshot();
            persistBlankProjectStorage(blankSnapshot);
            safeStorageRemove(STORAGE_KEYS.projectSnapshot);
            if (newProjectBtn) flashButton(newProjectBtn, 'Blank');
            showDeployStatus('Reloading the builder with a blank project...', 100, 'success');
            setTimeout(() => {
    ...
    window.__cherryCreateNewProject = createNewProject;
    ...
    function loadProjectSnapshot(snapshot) {
        ...
        const nextQuestionnaireSettings = {
            ...
            inputBarVisible: (snapshot?.questionnaire?.settings?.inputBarVisible
                ?? snapshot?.converse?.inputBarVisible
                ?? getDefaultQuestionnaireSettings().inputBarVisible) !== false
        };
        ...
        const converseSettings = sanitizeConverseSettings({
            ...(snapshot?.converse || {}),
            inputBarVisible: (snapshot?.converse?.inputBarVisible
                ?? snapshot?.questionnaire?.settings?.inputBarVisible
                ?? getDefaultConverseSettings().inputBarVisible) !== false
        });
        ...
        if (converseInputBarVisibleInput) converseInputBarVisibleInput.checked = converseSettings.inputBarVisible !== false;
        ...
        if (videoImageUrlInput) videoImageUrlInput.value = videoSettings.imageUrl;
        if (videoImageEnabledInput) videoImageEnabledInput.checked = videoSettings.imageEnabled;
        if (videoSizeInput) videoSizeInput.value = String(videoSettings.size);
        if (videoUrlInput || videoImageUrlInput || videoImageEnabledInput || videoSizeInput) {
            saveVideoSettings();
        } else {
        ...
        const authSettings = sanitizeAuthSettings(snapshot?.auth || {});
        if (authBasicUsernameInput) authBasicUsernameInput.value = authSettings.basicUsername;
        if (authBasicPasswordInput) authBasicPasswordInput.value = authSettings.basicPassword;
        if (authJwtTokenInput) authJwtTokenInput.value = authSettings.jwtToken;
        if (authHeaderNameInput) authHeaderNameInput.value = authSettings.headerName;
        if (authHeaderValueInput) authHeaderValueInput.value = authSettings.headerValue;
        updateAuthModeUi(authSettings.mode);
        saveAuthSettings(false);
    ...
    function setPrimaryInputMode(mode) {
        ...
        if (formInterface) formInterface.classList.toggle('form-interface--inline-step', showInlineNameForm);
    ...
    function toggleConversationUi(showConversation) {
        ...
        if (inputArea) {
            const shouldShowInput = showConversation && questionnaireSettings.inputBarVisible !== false;
            inputArea.hidden = !shouldShowInput;
            inputArea.style.display = shouldShowInput ? '' : 'none';
            inputArea.setAttribute('aria-hidden', shouldShowInput ? 'false' : 'true');
        }
        document.body.classList.toggle('conversation-started', showConversation);
    }

    function applyInputBarVisibility(visible) {
        const nextVisible = visible !== false;
        questionnaireSettings.inputBarVisible = nextVisible;
        currentConverseSettings = sanitizeConverseSettings({
            ...currentConverseSettings,
            inputBarVisible: nextVisible
        });
        if (questionShowInputBarInput) questionShowInputBarInput.checked = nextVisible;
        if (converseInputBarVisibleInput) converseInputBarVisibleInput.checked = nextVisible;
        if (inputArea) {
            const shouldShowInput = hasStartedConversation && nextVisible;
            inputArea.hidden = !shouldShowInput;
            inputArea.style.display = shouldShowInput ? '' : 'none';
            inputArea.setAttribute('aria-hidden', shouldShowInput ? 'false' : 'true');
        }
    ...
    function showIntroScreen() {
        hasStartedConversation = false;
        const introEnabled = !!currentConverseSettings?.intro?.enabled;
        if (!introEnabled) {
            startConversation();
            return;
        }
    ...
    function openGitDeployDrawer(show) {
        if (!gitDeployDrawer || !gitDeployOverlay) return;
        gitDeployDrawer.classList.toggle('open', show);
        gitDeployOverlay.classList.toggle('show', show);
        gitDeployDrawer.setAttribute('aria-hidden', show ? 'false' : 'true');
        syncDrawerUiState();
    }
    ...
    function openAuthDrawer(show) {
        if (!authDrawer || !authOverlay) return;
        authDrawer.classList.toggle('open', show);
        authOverlay.classList.toggle('show', show);
        authDrawer.setAttribute('aria-hidden', show ? 'false' : 'true');
        syncDrawerUiState();
    }
    ...
    function openVideoDrawer(show) {
        ...
        if (show) {
            ...
            if (videoImageUrlInput) {
                videoImageUrlInput.value = safeStorageGet(STORAGE_KEYS.videoImageUrl, getDefaultVideoSettings().imageUrl);
            }
            if (videoImageEnabledInput) {
                videoImageEnabledInput.checked = safeStorageGet(STORAGE_KEYS.videoImageEnabled, 'true') !== 'false';
            }
            if (videoSizeInput) {
                videoSizeInput.value = safeStorageGet(STORAGE_KEYS.videoSize, String(getDefaultVideoSettings().size));
            }
            previewVideoSettings();
        }
    }
    ...
    function loadQuestionSelection(index) {
        ...
        if (questionShowInputBarInput) questionShowInputBarInput.checked = questionnaireSettings.inputBarVisible !== false;
    }
    ...
    function saveSelectedQuestion() {
        ...
        questionnaireSettings.inputBarVisible = !!questionShowInputBarInput?.checked;
        applyInputBarVisibility(questionnaireSettings.inputBarVisible);
        applyQuestionnaireStyles();
        persistQuestionnaire();
        updateEmbedCode();
        renderQuestionList();
    ...
    function persistQuestionnaire() {
        ...
        safeStorageSet(STORAGE_KEYS.inputBarVisible, String(questionnaireSettings.inputBarVisible !== false));
        updateEmbedCode();
    }
    ...
    function getStoredHostingerSiteUrl() {
        return normalizePublicSiteUrl(safeStorageGet(STORAGE_KEYS.hostingerSiteUrl, ''));
    }

    function normalizeRemoteDir(value) {
        const trimmed = String(value || 'public_html').trim().replace(/\\/g, '/').replace(/^\/+|\/+$/g, '');
        if (!trimmed) return 'public_html';
        return trimmed.replace(/^public_html\/public_html(?:\/|$)/i, 'public_html/');
    }
    ...
    function normalizeEmbedBase(base, fallback) {
        ...
        if (IS_LOCAL_BUILDER && /\/\/(?:localhost|127\.0\.0\.1)(?::\d+)?/i.test(value)) {
            return getStoredHostingerSiteUrl() || fallback;
        }
        return value;
    }

    function getGithubStaticBase(repoPath, ref) {
        return `https://cdn.jsdelivr.net/gh/${repoPath}@${ref}/${GITHUB_STATIC_DIR}`;
    }

    function getGithubPagesBase(repoPath) {
        const [owner, repo] = String(repoPath || '').split('/');
        if (!owner || !repo) return '';
        return `https://${owner}.github.io/${repo}/${GITHUB_STATIC_DIR}`;
    }

    function joinBaseUrl(base, filename) {
        return `${String(base || '').replace(/\/+$/, '')}/${filename}`;
    }

    function stripEmbedBaseSuffix(value) {
        return String(value || '')
            .trim()
            .replace(new RegExp(`/${GITHUB_LOADER_FILENAME.replace('.', '\\.')}$`, 'i'), '')
            .replace(new RegExp(`/${GITHUB_RUNTIME_HTML_FILENAME.replace('.', '\\.')}$`, 'i'), '')
            .replace(/\/(?:auto-embed|embed|index)\.(?:js|html)$/i, '');
    }
    ...
    function setActiveEmbedBases(runtimeBase, appBase, commitSha = '') {
        ...
        if (embedJsUrlInput) {
            embedJsUrlInput.value = joinBaseUrl(runtimeBase || getActiveEmbedRuntimeBase(), GITHUB_LOADER_FILENAME);
        }
        if (embedCssUrlInput) {
            embedCssUrlInput.value = joinBaseUrl(appBase || getActiveEmbedAppBase(), GITHUB_RUNTIME_HTML_FILENAME);
        }
    }
    ...
    function buildHostingerSettingsFromInputs() {
        return {
            siteUrl: normalizePublicSiteUrl(hostingerSiteUrlInput?.value || safeStorageGet(STORAGE_KEYS.hostingerSiteUrl, '')),
            host: (hostingerHostInput?.value || safeStorageGet(STORAGE_KEYS.hostingerHost, '') || '').trim(),
            port: Number.parseInt(hostingerPortInput?.value || safeStorageGet(STORAGE_KEYS.hostingerPort, '21') || '21', 10) || 21,
            username: (hostingerUsernameInput?.value || safeStorageGet(STORAGE_KEYS.hostingerUsername, '') || '').trim(),
            password: (hostingerPasswordInput?.value || safeStorageGet(STORAGE_KEYS.hostingerPassword, '') || '').trim(),
            remoteDir: normalizeRemoteDir(hostingerRemoteDirInput?.value || safeStorageGet(STORAGE_KEYS.hostingerRemoteDir, 'public_html') || 'public_html'),
            secure: (hostingerSecureSelect?.value || safeStorageGet(STORAGE_KEYS.hostingerSecure, 'true') || 'true') !== 'false'
        };
    }

    function hydrateHostingerSettings() {
        if (hostingerSiteUrlInput) hostingerSiteUrlInput.value = getStoredHostingerSiteUrl();
        if (hostingerHostInput) hostingerHostInput.value = safeStorageGet(STORAGE_KEYS.hostingerHost, '');
        if (hostingerPortInput) hostingerPortInput.value = safeStorageGet(STORAGE_KEYS.hostingerPort, '21') || '21';
        if (hostingerUsernameInput) hostingerUsernameInput.value = safeStorageGet(STORAGE_KEYS.hostingerUsername, '');
        if (hostingerPasswordInput) hostingerPasswordInput.value = safeStorageGet(STORAGE_KEYS.hostingerPassword, '');
        if (hostingerRemoteDirInput) hostingerRemoteDirInput.value = normalizeRemoteDir(safeStorageGet(STORAGE_KEYS.hostingerRemoteDir, 'public_html') || 'public_html');
        if (hostingerSecureSelect) hostingerSecureSelect.value = safeStorageGet(STORAGE_KEYS.hostingerSecure, 'true') || 'true';

        const siteUrl = getStoredHostingerSiteUrl();
        if (siteUrl) {
            setActiveEmbedBases(siteUrl, siteUrl);
            updateEmbedCode();
        }
    }

    function saveHostingerSettings() {
        try {
            const settings = buildHostingerSettingsFromInputs();
            safeStorageSet(STORAGE_KEYS.hostingerSiteUrl, settings.siteUrl);
            safeStorageSet(STORAGE_KEYS.hostingerHost, settings.host);
            safeStorageSet(STORAGE_KEYS.hostingerPort, String(settings.port));
            safeStorageSet(STORAGE_KEYS.hostingerUsername, settings.username);
            safeStorageSet(STORAGE_KEYS.hostingerRemoteDir, settings.remoteDir);
            safeStorageSet(STORAGE_KEYS.hostingerSecure, String(settings.secure));
            if (settings.siteUrl) {
                setActiveEmbedBases(settings.siteUrl, settings.siteUrl);
                updateEmbedCode();
            }
            flashButton(saveHostingerSettingsBtn, 'Saved');
            setCredsStatus('Hostinger settings saved locally.', 'success');
        } catch (_) {
            setCredsStatus('Could not save Hostinger settings.', 'error');
        }
    }

    function saveHostingerPassword() {
        try {
            safeStorageSet(STORAGE_KEYS.hostingerPassword, (hostingerPasswordInput?.value || '').trim());
            flashButton(saveHostingerPasswordBtn, 'Saved');
            setCredsStatus('Hostinger password saved locally.', 'success');
        } catch (_) {
            setCredsStatus('Could not save Hostinger password.', 'error');
        }
    }

    function clearHostingerPassword() {
        try {
            safeStorageRemove(STORAGE_KEYS.hostingerPassword);
            if (hostingerPasswordInput) hostingerPasswordInput.value = '';
            flashButton(clearHostingerPasswordBtn, 'Cleared');
            setCredsStatus('Hostinger password cleared.', 'success');
        } catch (_) {
            setCredsStatus('Could not clear Hostinger password.', 'error');
        }
    }

    function validateHostingerSettings(settings, { requireSiteUrl = false } = {}) {
        if (!IS_LOCAL_BUILDER) {
            throw new Error('Hostinger deploy is only available while editing on localhost.');
        }
        if (!settings.host) throw new Error('Enter your Hostinger FTP host first.');
        if (!settings.username) throw new Error('Enter your Hostinger username first.');
        if (!settings.password) throw new Error('Enter your Hostinger password first.');
        if (!settings.remoteDir) throw new Error('Enter the Hostinger remote folder first.');
        if (requireSiteUrl && !/^https?:\/\//i.test(settings.siteUrl)) {
            throw new Error('Enter a valid public site URL starting with http:// or https://.');
        }
    }

    async function postHostingerApi(path, settings) {
        const requestInit = {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
        ...
        try {
            ...
            throw new Error('Could not reach the local builder server. Start or restart builder_server.py, then reload localhost.');
        }

        if (response.status === 404) {
            const fallbackPayload = await tryAlternateLocalApi(path, requestInit);
            if (fallbackPayload) return fallbackPayload;
            throw new Error(`Local deploy API not found on ${window.location.origin}. Restart builder_server.py and open the localhost tab it launches.`);
        }
        const payload = await response.json().catch(() => ({}));
    ...
    async function tryAlternateLocalApi(path, requestInit) {
        if (!IS_LOCAL_BUILDER) return null;

        const currentPort = window.location.port || (window.location.protocol === 'https:' ? '443' : '80');
        const candidateOrigins = LOCAL_BUILDER_PORTS
            .filter(port => port !== currentPort)
            .map(port => `${window.location.protocol}//${window.location.hostname}:${port}`);

        for (const origin of candidateOrigins) {
            try {
                const response = await fetch(`${origin}${path}`, requestInit);
                if (response.status === 404) continue;
                const payload = await response.json().catch(() => ({}));
                if (!response.ok || payload?.ok === false) continue;
                setCredsStatus(`Using local deploy API from ${origin}.`, 'success');
                return payload;
            } catch (_) {
                // Keep checking the next localhost port.
            }
        }
    ...
    async function testHostingerAccess() {
        try {
            const settings = buildHostingerSettingsFromInputs();
            validateHostingerSettings(settings);
            setCredsStatus('Testing Hostinger access...', 'success');
            const payload = await postHostingerApi('/api/test-hostinger', settings);
            const itemCount = Array.isArray(payload?.entries) ? payload.entries.length : 0;
            setCredsStatus(`Success: connected to ${settings.remoteDir}.`, 'success');
            showDeployStatus(`Hostinger ready: ${itemCount} visible items in ${settings.remoteDir}.`, 100, 'success');
        } catch (error) {
            setCredsStatus(error.message, 'error');
            showDeployStatus(error.message, 100, 'error');
        }
    }
    ...
    async function deployToHostinger() {
        try {
            const settings = buildHostingerSettingsFromInputs();
            validateHostingerSettings(settings, { requireSiteUrl: true });
            showDeployStatus(`Uploading to Hostinger ${settings.remoteDir}...`, 20, 'info');
            setCredsStatus('Deploying to Hostinger...', 'success');
            const payload = await postHostingerApi('/api/deploy-hostinger', settings);
            setActiveEmbedBases(settings.siteUrl, settings.siteUrl);
            updateEmbedCode();
            showDeployStatus(`Deploy complete: ${payload.uploaded?.length || 0} files uploaded.`, 100, 'success');
            setCredsStatus(`Hostinger deploy complete. Embed code now points to ${settings.siteUrl}.`, 'success');
        } catch (error) {
            showDeployStatus(`Hostinger deploy failed: ${error.message}`, 100, 'error');
            setCredsStatus(`Hostinger deploy failed: ${error.message}`, 'error');
            openCredsDrawer(true);
        }
    }

    function handlePrimaryDeployAction() {
        if (IS_LOCAL_BUILDER) {
            const settings = buildHostingerSettingsFromInputs();
            if (!(settings.host && settings.username && settings.password && settings.siteUrl)) {
                openCredsDrawer(true);
                setCredsStatus('Add your Hostinger settings, then press Deploy to Hostinger.', 'error');
                return;
            }
            deployToHostinger();
            return;
        ...
        openCredsDrawer(true);
        setCredsStatus('Hostinger deploy only runs from localhost. Use the GIT Deploy button for GitHub publishing.', 'error');
    }
    ...
    function getEmbedSnippetData() {
        const snapshot = getProjectSnapshot();
        const runtimeBase = stripEmbedBaseSuffix(embedJsUrlInput?.value || '') || getActiveEmbedRuntimeBase();
        const appBase = stripEmbedBaseSuffix(embedCssUrlInput?.value || '') || getActiveEmbedAppBase();
        const jsUrl = `${joinBaseUrl(runtimeBase, GITHUB_LOADER_FILENAME)}?v=4`;
        const appUrl = `${joinBaseUrl(appBase, GITHUB_RUNTIME_HTML_FILENAME)}?v=4`;
    ...
    function buildLauncherEmbedCode() {
        ...
        return `<script async src="${escapeEmbedAttr(jsUrl)}"
  data-cherry-loader="true"
  data-mode="floating"
  data-app-url="${escapeEmbedAttr(appUrl)}"
  data-project="${escapeEmbedAttr(encodedProject)}"
  data-webhook="${escapeEmbedAttr(webhook)}"
  data-label="${escapeEmbedAttr(label)}"
  data-subtext="${escapeEmbedAttr(subtext)}"
  data-subtext-display="${escapeEmbedAttr(subtextDisplay)}"
  data-icon-url="${escapeEmbedAttr(iconUrl)}"
  data-icon-size="${escapeEmbedAttr(iconSize)}"
  data-icon-shape="${escapeEmbedAttr(shape)}"
  data-icon-anim="${escapeEmbedAttr(anim)}"
  data-icon-3d="${escapeEmbedAttr(String(is3d))}"
></script>`;
    }
    ...
    function buildInlineEmbedCode() {
        ...
        return `<div id="${escapeEmbedAttr(containerId)}"></div>
<script async src="${escapeEmbedAttr(jsUrl)}"
  data-cherry-loader="true"
  data-mode="inline"
  data-target="${escapeEmbedAttr(containerId)}"
  data-app-url="${escapeEmbedAttr(appUrl)}"
  data-project="${escapeEmbedAttr(encodedProject)}"
  data-webhook="${escapeEmbedAttr(webhook)}"
></script>`;
    ...
    function applyLauncher(label, subtext, subtextDisplay, iconUrl, shape, anim, is3d, iconSize) {
        ...
        const size = Math.max(18, Math.min(36, parseInt(iconSize, 10) || 24));
    ...
    function hydrateGithubSettings() {
        try {
            ...
            setGitDeployStatus(token ? 'Ready to deploy.' : 'Paste your GitHub token once, then press Deploy.', token ? 'success' : 'error');
        } catch (e) {
            setGitDeployStatus('Local storage not available in this context.', 'error');
        }
    }
    ...
    function saveGithubSettings() {
        try {
            ...
            setGitDeployStatus('Repo and branch saved.', 'success');
        } catch (e) {
            setGitDeployStatus('Could not save (storage blocked).', 'error');
        }
    }
    ...
    function saveGithubToken() {
        try {
            ...
            setGitDeployStatus('Token saved locally.', 'success');
        } catch (e) {
            setGitDeployStatus('Could not save token (storage blocked).', 'error');
        }
    }
    ...
    function clearGithubToken() {
        try {
            ...
            setGitDeployStatus('Token cleared.', 'success');
        } catch (e) {
            setGitDeployStatus('Could not clear token.', 'error');
        }
    }
    ...
    if (saveHostingerSettingsBtn) saveHostingerSettingsBtn.addEventListener('click', saveHostingerSettings);
    if (saveHostingerPasswordBtn) saveHostingerPasswordBtn.addEventListener('click', saveHostingerPassword);
    if (clearHostingerPasswordBtn) clearHostingerPasswordBtn.addEventListener('click', clearHostingerPassword);
    if (testHostingerAccessBtn) testHostingerAccessBtn.addEventListener('click', testHostingerAccess);
    ...
    function setGitDeployStatus(message, tone = 'success') {
        if (!gitDeployStatus) return;
        gitDeployStatus.textContent = message;
        gitDeployStatus.classList.remove('success', 'error');
        gitDeployStatus.classList.add(tone);
    }
    ...
    async function testGithubAccess() {
        ...
        if (!repoPath) {
            setGitDeployStatus('Enter a valid GitHub repo URL (e.g., https://github.com/owner/repo).', 'error');
            return;
        }
        const token = (githubTokenInput?.value || '').trim() || localStorage.getItem(STORAGE_KEYS.token);
        if (!token) {
            setGitDeployStatus('Add a GitHub token first.', 'error');
            return;
        }
        ...
        setGitDeployStatus('Testing access...', 'success');
        ...
        try {
            ...
            if (repoResp.status === 404) {
                setGitDeployStatus('Repo not found (check URL or token scope).', 'error');
                return;
            }
            if (repoResp.status === 401) {
                setGitDeployStatus('Unauthorized: token invalid or missing scopes.', 'error');
                return;
            }
            if (!repoResp.ok) {
                setGitDeployStatus(`Failed (status ${repoResp.status}) getting repo.`, 'error');
                return;
            }
            ...
            if (resp.status === 200) {
                setGitDeployStatus(`Success: can read ${repoPath} @ ${targetBranch}.`, 'success');
            } else if (resp.status === 404) {
                setGitDeployStatus('Repo or branch not found (check URL/branch/token scopes).', 'error');
            } else if (resp.status === 401) {
                setGitDeployStatus('Unauthorized: token invalid or missing scopes.', 'error');
            } else {
                setGitDeployStatus(`Failed (status ${resp.status}).`, 'error');
            }
        } catch (e) {
            setGitDeployStatus('Network error while contacting GitHub.', 'error');
        }
    }
